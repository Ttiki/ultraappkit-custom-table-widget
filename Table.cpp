...
namespace UltraEngine
{
	...
	void Table::ModifyItem(const int index, const WString& text)
	{
	...
	shared_ptr<Widget> CreateTable(const int x, const int y, const int width, const int height, shared_ptr<Widget> parent, const TableStyle style)
	{
		auto widget = make_shared<Table>();
		...
		widget->Invalidate();
		widget->slider = CreateSlider(width - dynamic_pointer_cast<Table>(widget)->sliderwidth * scale, 0, dynamic_pointer_cast<Table>(widget)->sliderwidth * scale, height - 0, widget, SLIDER_VERTICAL | SLIDER_SCROLLBAR);
		widget->slider->Hide();
	...
	void Table::MouseWheel(const int delta, const int x, const int y)
	{
		if (!slider->Hidden())
		{
	...
	void Table::MouseEnter(const int x, const int y)
	{
		hoverstate = true;
		Redraw();
	}

	void Table::MouseLeave(const int x, const int y)
	{
		hoverstate = false;
		Redraw();
	}

	int Table::ItemHeight()
	{
		return Round(float(gui->GetLineHeight(font, fontscale, fontweight)) * 1.5f);
	}

	void Table::MouseMove(const int x, const int y)
	{
	...
	void Table::DoubleClick(const MouseButton button, const int x, const int y)
	{
	...
	void Table::MouseDown(const MouseButton button, const int x, const int y)
	{
	...
	void Table::MouseUp(const MouseButton button, const int x, const int y)
	{
	}

	void Table::GainFocus()
	{
	}

	void Table::LoseFocus()
	{
		//auto combobox = this->combobox.lock();
		//if (combobox)
		//{
	...
	void Table::KeyDown(const KeyCode key)
	{
		auto* items = &this->items;
	...
	void Table::SelectItem(const int index)
	{
		auto* items = &this->items;
		if (filtertext.size() != 0) items = &filtereditems;
	...
	void Table::SetShape(int x, int y, int width, int height)
	{
	...
	bool Table::ProcessEvent(const Event& e)
	{
		if (e.source == slider)
		{
	...
	int Table::AddItem(const WString& text, const bool selected)
	{
		rebuild = true;
	...
	void Table::FilterItems(const WString& filter)
	{
		Lock();
	...
	void Table::Draw(const int x, const int y, const int width, const int height)
	{
